...
from baselines import MLP,CNN1D_DeepGoPlus
from networks import MVFF,DeepCNN
...
def main(**kwargs):
    ...
    n_view = int(Counter(args.feats_type)['_']) + 1
    print("[*] Selected device:", device)
    print("[*] Using [%s] features." % args.feats_type)
    # dataPre
    input_dim,input_dims = pass_config(device,args,opt)
    terms_dict,terms,train_df,valid_df,test_df,num_classes = get_data('data/'+ args.datasets,args.namespace,split=0.9)

    # 保存预测结果prediction.pkl 模型及 模型断点
    ...
    logger_file = "models/" +args.datasets+"/"+ args.namespace.upper() + f"/{args.datasets}_{args.namespace.upper()}_{args.net_type}_{args.feats_type}_result.txt"
    F_txt = open(logger_file, "a+")

    # 网络初始化
    if args.net_type == 'MLP':
        net = MLP(input_dim=input_dim, hidden_dim=512, num_classes=num_classes).to(device)

    elif args.net_type == "DeepGOCNN":
        net = CNN1D_DeepGoPlus(input_dim=input_dim, num_filters=16 * [512],num_classes=num_classes).to(device)

    elif args.net_type == "DeepCNN":
        net = DeepCNN(input_dim=input_dim, hidden_dim=512,num_classes=num_classes).to(device)

    elif args.net_type == "MVFFNet":
        net = MVFF(device,input_dims=input_dims,num_classes=num_classes,hidden_dim=512,n_view=n_view).to(device)
    ...
    train_set = CNN1DDataset(df=train_df, feats_dir=args.feats_dir, terms_dict=terms_dict,
                             feats_type=args.feats_type)
    train_loader = pyDataLoader(train_set, batch_size=args.batch_size,  num_workers=4,
                                collate_fn=cnn1d_collate)

    valid_set = CNN1DDataset(df=valid_df, feats_dir=args.feats_dir, terms_dict=terms_dict,
                             feats_type=args.feats_type)
    valid_loader = pyDataLoader(valid_set, batch_size=args.batch_size, num_workers=4,
                                collate_fn=cnn1d_collate)

    test_set = CNN1DDataset(df=test_df, feats_dir=args.feats_dir, terms_dict=terms_dict,
                            feats_type=args.feats_type)
    test_loader = pyDataLoader(test_set, batch_size=args.batch_size, num_workers=4,
                               collate_fn=cnn1d_collate)
    ...
    for name in model_list:
        ...
        if epoch_num != num_epoch:
            ...
            break
        epoch_num,test_loss, test_rocauc,test_fmax, y_true, y_pred_sigm = test(opt,
        device=device, net=net, criterion=criterion, model_file=args.model_dir, test_loader=test_loader, save_file=args.out_file)
        print("[*] Loaded checkpoint at epoch {} for Testing\tTest loss:{:.4f}\tTest ROC AUC:{:.4f}\tTest Fmax:{:.4f}".format(epoch_num,test_loss, test_rocauc,test_fmax), file=F_txt)
